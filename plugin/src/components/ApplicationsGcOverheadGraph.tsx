...
import { populateGCOverheadMetrics } from '../services/CamelService';
...
const ApplicationGcPauseGraph: React.FC<{applications: Application[] }> = ({ applications }) => {
  ...
  useEffect(() => {
    ...
    applications.filter(app => app && app.metrics.cpu).forEach((app, index) => {
      populateGCOverheadMetrics(app).then((appWithMetrics) => {
        newData[index] = appWithMetrics.metrics.gcOverhead;
